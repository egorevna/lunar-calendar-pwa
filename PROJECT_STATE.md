...
- активная задача — Task 10.9, Terms / Decans / Degree Rulers Display Helper;
...
2. Работать только над Task 10.9 после отдельной команды пользователя.
3. Объяснить минимальный план изменения.
4. Внести только необходимые изменения.
5. В Task 10.9 создавать только pure display helper for already computed terms / decans / degree ruler lookup results; не делать UI, новые lookup rules, Table 7 rows, Vronsky degree rows, exact exaltation degrees, натальную карту, персональные транзиты, дома, ASC/MC, интерпретации или фейковые личные расчеты.
