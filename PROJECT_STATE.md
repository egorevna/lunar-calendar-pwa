...
Не начинать Task 1.3 без отдельной команды пользователя.
...
```txt
Выполнить Task 1.3 — Improve Moon Aspects Block.
```

Перед реализацией Codex должен:

1. Найти, где сейчас отображается строка аспекта VOC.
2. Найти, какие данные о последнем аспекте перед VOC доступны.
