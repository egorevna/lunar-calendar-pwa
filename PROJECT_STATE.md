...
- активная задача — Task 10.7c, Degree Rulers Table 6 Manual Verification;
...
2. Работать только над Task 10.7c после отдельной команды пользователя.
3. Объяснить минимальный план изменения.
4. Внести только необходимые изменения.
5. В Task 10.7c сверять только existing Table 6 transcription draft against source image; не делать active dataset, UI, lookup engine, Table 7 rows, Vronsky degree rows, exact exaltation degrees, натальную карту, персональные транзиты, дома, ASC/MC, интерпретации или фейковые личные расчеты.
