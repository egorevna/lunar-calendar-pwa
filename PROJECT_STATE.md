...
```txt
Выполнить Task 1.10 — Add Hidden Debug Screen.
```

Перед реализацией Codex должен:

1. Найти, какие расчетные данные нужны для скрытого debug-экрана.
2. Найти минимальный способ доступа к debug-экрану без показа обычному пользователю.
