...
```txt
Sprint 12 — House Cusps + Pars Fortuna + Basic Arabic Parts active
```
...
- Sprint 11 закрыт; Sprint 12 начат;
...
- Task 12.5 завершена: добавлен data-only formula dataset / source decision layer `src/arabicPartsData.js`; `pars-fortuna` остается единственной active verified формулой, Lot of Spirit и дополнительные Arabic Parts deferred until source verification;
- следующая активная задача — Task 12.6 — Basic Arabic Parts Engine / Fixtures;
...
2. Работать только над Task 12.6 после отдельной команды пользователя.
3. Объяснить минимальный план изменения.
4. Внести только необходимые изменения.
5. Не начинать Task 12.6 без отдельного явного запроса.
