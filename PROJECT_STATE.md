...
```txt
Sprint 16 — Midpoints / Antiscia Foundation active
```
...
Sprint 10 закрыт. Sprint 11 — Houses / ASC / MC закрыт. Task 11.1 добавила `SPRINT_11_PLAN.md` и `HOUSES_ASC_MC_STRATEGY.md`; они фиксируют расчетную стратегию, guardrails и порядок задач Sprint 11. Task 11.2 добавила pure `src/housesInputGuardrails.js` readiness / guardrails layer and `test/housesInputGuardrails.test.js`. Task 11.3 добавила pure `src/ascMc.js` ASC / MC calculation engine and `test/ascMc.test.js`. Task 11.4a обновила house systems strategy / dependency audit как docs-only. Task 11.4b добавила pure `src/wholeSignHouses.js` Whole Sign houses engine and `test/wholeSignHouses.test.js`. Task 11.4c добавила pure `src/equalHouseHouses.js` Equal House / Равнодомная houses engine and `test/equalHouseHouses.test.js`. Task 11.4d добавила pure `src/placidusHouses.js` Placidus integration gate and `test/placidusHouses.test.js`. Task 11.4d2 activated browser-safe Placidus calculation in `src/placidusHouses.js` using static benchmark fixtures from local `swisseph.swe_houses`. Task 11.4e добавила pure `src/houseSystemResolver.js` selected-system router and `test/houseSystemResolver.test.js`. Task 11.5 добавила cross-system validation fixtures and tests for Whole Sign, Equal House and Placidus. Task 11.6 добавила pure planet-in-house assignment layer for selected house systems. Task 11.7 добавила pure Houses / ASC / MC display helper for ready house, angle and planet-in-house results. Task 11.8 добавила collapsible `Дома и углы карты` UI block inside `Мои карты`: block is collapsed by default, appears after `Термы, деканы и градусы`, shows selected house system, angles, houses and planet assignments when ready, and safe fallback/unsupported messages when not ready. Post-11.8 UI/copy fix removed duplicate fallback message rendering and keeps the header to title plus `Показать` / `Скрыть`. Task 11.8b добавила ручной ввод широты/долготы места рождения в профиль, хранение в `birthPlace.coordinates`, paired/range validation, city-level helper copy and no geocoding / no external lookup. Task 11.8c verified the Moscow 1981 Placidus ready-state case after manual coordinates and added regression tests against duplicated Placidus cusps / UI rows; follow-up runtime-path hardening checks UTC conversion, ASC / MC, raw Placidus cusps, `housesForProfile` rows and `profileUi` rows together, fixes mixed-shape profile coordinate priority, fixes a Placidus endpoint-root cusp collapse edge case, enforces single-source display consistency for Placidus angles/houses, avoids a second independent house calculation for planet assignments in `housesForProfile`, and bumps PWA cache to `lunar-calendar-v83`. Task 11.9 добавила safe `Houses / ASC / MC UI Debug` section for `?debug=1`: debug exposes only active profile id/name, readiness booleans, selected system, counts, capabilities and privacy flags, without raw birth data, coordinates, UTC, timezone value or full houses/cusps/assignments arrays. Task 11.10 completed final hardening: guardrails, house-system separation, Placidus validation, coordinate/profile shape, architecture boundaries, privacy, UI/debug and PWA cache were rechecked; no code fixes were needed. Task 11.10-fix updated visible zodiac position formatting to nearest-minute rounding, then Task 11.10-fix-2 corrected the policy: user-facing ASC / MC and house cusp text now shows degree-minute-second precision without nearest-minute rounding while numeric longitudes remain precise. Task 11.10-fix-3 aligned Placidus precision with the local Swiss benchmark: Placidus now uses true obliquity for cusp anchors and derives displayed ASC / MC / DSC / IC from cusps 1 / 10 / 7 / 4. No interpretations were added. Sprint 12 — House Cusps + Pars Fortuna + Basic Arabic Parts закрыт. Task 12.1 добавила strategy docs: `SPRINT_12_PLAN.md`, `HOUSE_CUSPS_PARS_FORTUNA_STRATEGY.md` и `PARS_FORTUNA_ARABIC_PARTS_FORMULA_POLICY.md`. Task 12.2 добавила canonical house cusp layer `src/houseCusps.js` and static/manual fixtures for Whole Sign, Equal House and Placidus. Task 12.3 добавила day/night chart status engine `src/dayNightChart.js` using geometric Sun altitude and explicit boundary handling. Task 12.4 добавила pure Pars Fortuna engine `src/parsFortuna.js` with verified day/night formulas and static/manual fixtures. Task 12.5 добавила data-only Arabic Parts formula dataset/source decision layer `src/arabicPartsData.js`; Task 12.5b verified Lot of Spirit as an active formula row with day `ASC + Sun - Moon` and night `ASC + Moon - Sun`; Task 12.6 добавила pure Basic Arabic Parts engine `src/arabicParts.js` for active verified formulas only. The engine calculates Pars Fortuna and Lot of Spirit from numeric ASC, Sun and Moon longitudes plus explicit day/night chart status; deferred Arabic Parts stay inactive and do not produce ready values. Task 12.7 добавила pure Lots / Arabic Parts house assignment layer `src/arabicPartsHouseAssignment.js`: active lots are assigned to selected-system canonical cusps using numeric longitudes, half-open spans and exact-cusp boundary policy. Task 12.8 добавила pure Lots / Arabic Parts display helper `src/arabicPartsDisplay.js`: it formats already calculated Pars Fortuna / Lot of Spirit results, optional house assignment labels, day/night labels and safe fallback states. Task 12.9 добавила collapsible user-facing UI block `Жребии и арабские части` inside `Мои карты` after `Дома и углы карты`; block is collapsed by default, shows day/night chart label, Pars Fortuna, Lot of Spirit and house labels when ready, and safe fallback when not ready. Task 12.10 добавила safe `Arabic Parts UI Debug` section for `?debug=1`: debug exposes only active profile id/name, readiness booleans, chart sect status/label, active/deferred formula keys, counts, capabilities and privacy flags, without raw birth data, coordinates, UTC, raw longitudes, formula operand arrays, provider payloads or full result arrays. Task 12.11 completed final hardening: canonical cusps, day/night status, Pars Fortuna, Lot of Spirit, deferred Arabic Parts, assignment, UI/debug, privacy and architecture boundaries were rechecked; no code fixes were needed. PWA cache is `lunar-calendar-v90`. Sprint 13 — Special Points Foundation начат. Task 13.1 добавила strategy docs: `SPRINT_13_PLAN.md`, `SPECIAL_POINTS_STRATEGY.md` и `SPECIAL_POINTS_SOURCE_POLICY.md`. Task 13.2 добавила `LUNAR_NODES_SOURCE_POLICY.md`: активная система узлов — `mean-lunar-node` / `lunar-nodes-mean`, true node deferred, Южный узел выводится как `normalize(Северный узел + 180°)`, validation plan uses static local Swiss Ephemeris `SE_MEAN_NODE` benchmark fixtures. Task 13.3 добавила pure `src/lunarNodes.js` mean Lunar Nodes engine and static fixtures/tests; Северный узел рассчитывается по active mean node policy, Южный узел выводится только как Северный узел + 180°, координаты рождения не требуются, true node / Lilith / Selena / interpretations remain deferred or forbidden. Task 13.4 добавила pure `src/lunarNodesHouseAssignment.js` assignment layer: Северный / Южный узлы назначаются в Whole Sign, Equal House и Placidus дома через canonical cusps, numeric longitude, half-open spans and exact-cusp boundary policy; Lunar Nodes calculation, house engines, UI/display/debug and provider calculations were not changed. Task 13.5 добавила `LILITH_SOURCE_DECISION.md`: Lilith status is `source-verified-for-implementation`; Mean Black Moon Lilith / Mean Lunar Apogee is the active target for Task 13.6; local Swiss Ephemeris `SE_MEAN_APOG` / `swe_calc_ut` is the static benchmark oracle only; True / Osculating and interpolated Lilith remain deferred. Task 13.6 добавила pure `src/lilith.js` Mean Lilith engine and static fixtures/tests; реализована только Средняя Лилит / Mean Lunar Apogee, координаты рождения не требуются, true / osculating / interpolated Lilith, Selena, house assignment, UI/debug and interpretations remain deferred or forbidden. Task 13.7 добавила `SELENA_SOURCE_DECISION.md`: Selena status is `source-verified-for-implementation`; active source system is `selena-white-moon`; calculation method is `swisseph-seorbel-white-moon-linear-elements`; local Swiss Ephemeris `SE_WHITE_MOON` / `swe_calc_ut` is the static benchmark oracle only. Task 13.8 добавила pure `src/selena.js` Selena / White Moon engine and static fixtures/tests; реализована только выбранная Swiss Ephemeris seorbel source system, Selena помечена как `fictitious-calculated-point`, координаты рождения не требуются, alternate Selena variants / Lunar Nodes changes / Lilith changes / UI/debug / house assignment / interpretations не добавлены. Task 13.9 завершена: добавлен pure `src/specialPointsDisplay.js` display helper for already calculated Special Points; it formats North/South Nodes, Mean Lilith, Selena, optional Lunar Nodes house labels, safe fallback states and the Selena fictitious / hypothetical note without calculating points, assigning houses, rendering UI/debug or adding interpretations. Task 13.10 добавила collapsible user-facing UI block `Особые точки карты` inside `Мои карты` after `Жребии и арабские части`; block is collapsed by default, shows Lunar Nodes, Mean Lilith and Selena when ready, shows Lunar Nodes house assignment when ready, includes the safe Selena fictitious / hypothetical calculated-point note, and adds no interpretations. PWA cache is `lunar-calendar-v91`. Task 14.4 добавила pure Fixed Star position / epoch engine with exact-source, interpolation, extrapolation and wrap-around handling. Task 14.5 добавила pure Fixed Star target resolver `src/fixedStarTargets.js`: active targets are natal planets + ASC / MC / DSC / IC, deferred targets stay metadata-only, and no conjunction engine, UI/debug or interpretations were added. Task 14.6 добавила pure Fixed Star conjunction engine `src/fixedStarConjunctions.js`: relationship is conjunction only, global orb is `1°00′`, targets are natal planets + ASC / MC / DSC / IC, wrap-around and inclusive orb boundary are supported, and no display/UI/debug or interpretations were added. Task 14.7 добавила Fixed Stars validation / cross-check fixtures and tests across catalog, source metadata, epoch columns, position policy, target policy, orb policy, conjunction boundaries, privacy and strict runtime/file exclusions; no UI/display/debug or interpretations were added. Task 14.8 добавила pure Fixed Stars display helper `src/fixedStarsDisplay.js`; Task 14.9 добавила collapsible user-facing UI block `Неподвижные звезды` inside `Мои карты` after `Особые точки карты`, with ready/noHits/partial/notReady behavior and one-time source/orb notes. Task 14.10 добавила safe `Fixed Stars Debug` section for `?debug=1`, exposing only catalog/policy/pipeline counts, statuses and guardrails without raw profile data or full arrays. Task 14.11 completed final hardening/regression for source policy, catalog, positions, targets, conjunctions, validation, display, UI, debug, privacy and cache; no code fixes were needed. Sprint 14 закрыт. Task 15.1 добавила Sprint 15 Arabic Parts Expansion Pack strategy docs; Task 15.2 added `ARABIC_PARTS_SOURCE_CORPUS_DECISION.md`; Task 15.2b accepted Vronsky Table 17 as the partial primary/only source corpus and recorded day-birth source-visible rows without activating new Lots; Task 15.2c added `ARABIC_PARTS_VRONSKY_SCOPE_SELECTION.md`, selected the first Vronsky simple day-only display-safe implementation pack for Task 15.3, deferred sensitive labels and complex operands, kept night formulas missing/unverified, and activated no new Lots; Task 15.3 added 12 inactive pending-engine Vronsky simple Arabic Points dataset rows and fixtures; Task 15.4 added the explicit day-only Vronsky simple Arabic Points engine / fixtures; Task 15.5 added explicit Vronsky house assignment / display integration and the explicit profile helper without UI/debug; Task 15.6 added the Vronsky Arabic Points UI subsection inside the existing `Жребии и арабские части` block with day-ready rows, safe night/boundary/unknown fallback, one-time source/day-only notes, and no formula changes; Task 15.7 added safe Vronsky Arabic Points Debug / QA guardrails for `?debug=1`; Task 15.8 completed final Vronsky Arabic Points hardening / regression; Sprint 15 закрыт. Task 16.1 added / accepted `SPRINT_16_PLAN.md`, `MIDPOINTS_ANTISCIA_STRATEGY.md` and `MIDPOINTS_ANTISCIA_SOURCE_POLICY.md`; Task 16.2 added `MIDPOINTS_ANTISCIA_FORMULA_SCOPE_DECISION.md` and accepted source-verified midpoint / antiscia / contra-antiscia formula and scope policy; Task 16.3 added pure `src/midpointTargets.js` target resolver with static/manual fixtures and tests; active midpoint targets are natal planets only, target count is `10`, deterministic unordered pair definitions count is `45`, deferred target sets stay metadata-only, and no midpoint longitude calculation, antiscia calculation, UI/debug, provider, package or service worker changes were made; Task 16.4 added pure `src/midpoints.js` midpoint engine and fixtures; Task 16.5 added midpoint validation/cross-check fixtures and tests; Task 16.6 added pure `src/antisciaTargets.js` target resolver / fixtures for natal planets + ASC / MC / DSC / IC, target count `14`, without antiscia calculations; Task 16.7 added pure `src/antiscia.js` Antiscia / Contra-antiscia engine and fixtures for the same active target scope; next active task is Task 16.8 — Antiscia Validation / Cross-Checks.

Current checkpoint update:

- Task 13.11 завершена: added safe `Special Points UI Debug` for `?debug=1`;
- Special Points debug exposes safe status/readiness/source statuses/counts/capabilities/privacy flags only;
- raw birth data, coordinates, UTC, raw longitudes, full profile JSON, full points/assignments/cusps arrays and provider payloads are not exposed;
- Post-13.11 UX/copy fix keeps the user-facing `Особые точки карты` block clean: calculated rows render first, then one shared deduplicated notes list at the bottom;
- PWA cache is `lunar-calendar-v94`;
- Task 13.12 завершена: Sprint 13 hardening rechecked Mean Lunar Nodes, Lunar Nodes house assignment, Mean Lilith, Selena / White Moon, Special Points UI/debug, privacy and architecture boundaries;
- Sprint 13 закрыт;
- true node, true / osculating / interpolated Lilith and alternate Selena source systems remain deferred;
- Task 14.1 завершена: added / accepted `SPRINT_14_PLAN.md`, `FIXED_STARS_STRATEGY.md` and `FIXED_STARS_SOURCE_POLICY.md` as Sprint 14 strategy docs;
- Sprint 14 начат: Fixed Stars are source-gated; no catalog row may be activated from memory; no hidden orb; no mixed epochs; conjunction-only is the initial relationship policy; no interpretations are allowed;
- Task 14.2 завершена: added `FIXED_STARS_SOURCE_DECISION.md`; primary astrology source is Vronsky Table 18; validation policy is Vronsky primary plus Swiss / modern validation where possible; candidate-only subset is Алголь, Альдебаран, Ригель, Бетельгейзе, Сириус, Канопус, Регул, Спика, Арктур, Антарес, Вега, Альтаир and Фомальгаут; global conjunction orb is `1°00′`; first target set is natal planets + ASC / MC / DSC / IC;
- Task 14.3 завершена: added data-only `src/fixedStarsData.js` and static/manual fixtures/tests; 13 active rows were manually verified from Vronsky Table 18 reference photos; candidate row count is 0; Vronsky 1950 / 1970 / 1990 source columns are preserved and the 1990 column is the initial reference epoch; no OCR import, no rows from memory, no position/precession engine, no conjunction engine and no interpretations were added;
- Task 14.4 завершена: added pure `src/fixedStarPositions.js` position / epoch engine and static/manual fixtures/tests; exact Vronsky source epochs are preserved, in-range dates use explicit interpolation, out-of-range dates use explicit extrapolation, wrap-around near 0° Aries is handled safely, and no conjunction engine, target resolver, UI/debug or interpretations were added;
- Task 14.5 завершена: added pure `src/fixedStarTargets.js` target resolver, static/manual fixtures and tests; active target sets are natal planets and angles (ASC / MC / DSC / IC); house cusps, Lunar Nodes, Lilith, Selena, Pars Fortuna, Lot of Spirit, Arabic Parts and custom points remain deferred metadata-only; no conjunction engine, UI/debug or interpretations were added;
- Task 14.6 завершена: added pure `src/fixedStarConjunctions.js` conjunction engine, static/manual fixtures and tests; only conjunctions are calculated, global orb `1°00′` is inclusive, shortest angular distance handles 359° / 0° Aries wrap-around, ready / partial / noHits / notReady states are preserved, and no display/UI/debug or interpretations were added;
- Task 14.7 завершена: added Fixed Stars validation / cross-check fixtures and tests across catalog, source metadata, epoch columns, position policy, target policy, orb policy, conjunction boundaries, privacy and strict runtime/file exclusions; no production modules, UI/display/debug or interpretations were added;
- Task 14.8 завершена: added pure `src/fixedStarsDisplay.js` display helper for already calculated Fixed Star conjunction hits, ready/noHits/notReady states and safe source/orb notes; no calculations, UI/debug or interpretations were added;
- Task 14.9 завершена: added collapsible user-facing `Неподвижные звезды` UI block inside `Мои карты` after `Особые точки карты`; it shows conjunction hits, noHits, partial and notReady states, source/orb notes once at the bottom, and no interpretations/debug;
- Task 14.10 завершена: added safe `Fixed Stars Debug` for `?debug=1`; debug shows only catalog/policy/pipeline counts, statuses and guardrails, without raw birth data, coordinates, UTC, provider payloads, full arrays or interpretations;
- PWA cache is `lunar-calendar-v95`;
- Task 14.11 завершена: final hardening / regression rechecked source policy, catalog, positions, targets, conjunctions, validation fixtures, display, UI, debug / QA guardrails, privacy, architecture boundaries and cache; no code fixes were needed;
- Sprint 14 закрыт;
- Task 15.1 завершена: added / accepted `SPRINT_15_PLAN.md`, `ARABIC_PARTS_EXPANSION_STRATEGY.md` and `ARABIC_PARTS_EXPANSION_SOURCE_POLICY.md` as Sprint 15 strategy docs;
- Sprint 15 начат: Arabic Parts expansion is source-gated; active formulas remain `pars-fortuna` and `lot-of-spirit`; candidate Lots are `lot-of-eros`, `lot-of-necessity`, `lot-of-basis` and `lot-of-exaltation`;
- no candidate Lot may be activated from memory; source/tradition verification is required; conflicts stay inactive as `needsReview` or `deferred`; no interpretations are allowed;
- Task 15.2 завершена as blocked/source-materials-required: `ARABIC_PARTS_SOURCE_CORPUS_DECISION.md` recorded that no local verified source corpus was available for Eros / Necessity / Basis / Exaltation;
- Task 15.2b завершена: accepted Vronsky, Том 1, Приложение 2, Таблица 17 — `Арабские точки` as the partial primary/only Sprint 15 source corpus; day-birth rows are source-visible, night-birth formulas are not verified, and no new Lots were activated;
- Sprint 15 is reframed around Vronsky Table 17 Arabic Points; `Pars amoris / точка любви` is source-visible as `pars-amoris` candidate and must not be mapped automatically to `lot-of-eros`;
- Task 15.2c завершена: added `ARABIC_PARTS_VRONSKY_SCOPE_SELECTION.md`, selected the first simple day-only display-safe Vronsky implementation pack for Task 15.3, deferred sensitive labels and complex operands, and kept night formulas missing / not verified;
- Task 15.3 завершена: added 12 source-tracked Vronsky simple Arabic Points rows to `src/arabicPartsData.js` as inactive / pending-engine metadata with fixtures and tests; active calculated formulas remain exactly `pars-fortuna` and `lot-of-spirit`;
- Task 15.4 завершена: added explicit Vronsky simple Arabic Points engine expansion in `src/arabicParts.js`; the 12 selected Vronsky Table 17 rows are `engineReady` and calculate only through explicit Vronsky APIs for `chartSect: day`; night, boundary and unknown chart sect fail closed as `notReady`; missing operands produce safe partial / notReady rows; default Arabic Parts output remains only `pars-fortuna` and `lot-of-spirit`;
- Task 15.5 завершена: added explicit Vronsky Arabic Points house assignment / display integration; `src/arabicPartsHouseAssignment.js` can assign the 12 selected Vronsky points to selected canonical house cusps, `src/arabicPartsDisplay.js` can format Vronsky rows with optional `· N дом`, and `src/vronskyArabicPartsForProfile.js` composes calculation, assignment and display without wiring into normal UI;
- Vronsky assignment uses numeric longitude only, canonical selected-system cusps, half-open spans `[cusp, nextCusp)`, exact cusp belongs to the starting house and wrap-around is supported;
- Vronsky display remains factual/day-only: day rows display when ready, night/boundary/unknown chart sect states return safe fallback messages, and no interpretations are added;
- existing active formulas remain `pars-fortuna` and `lot-of-spirit`;
- Task 15.6 завершена: Vronsky Arabic Points UI subsection added inside the existing `Жребии и арабские части` block; day charts show the 12 selected Vronsky rows when ready, night/boundary/unknown chart sect states show safe fallback messages, source/day-only notes appear once at the bottom, and no formulas/debug/interpretations were changed;
- Task 15.7 завершена: added safe `Vronsky Arabic Points Debug` for `?debug=1`; debug shows Vronsky-only source corpus, selected row count, day-only policy, night formula status, pipeline counts and QA guardrails only;
- Vronsky debug does not expose raw birth data, UTC, timezone values, coordinates, provider payloads, formula operand arrays, full profile JSON or full result arrays;
- Vronsky debug confirms old deferred Lots remain inactive and no interpretations are added;
- PWA cache is `lunar-calendar-v97`;
- Task 15.8 завершена: final hardening / regression rechecked the Vronsky Arabic Points source corpus, source materials, scope selection, 12-row dataset, explicit engine, day-only policy, house assignment, display, UI, debug, privacy, architecture boundaries and service worker cache;
- no code fixes were needed in Task 15.8;
- Sprint 15 закрыт;
- финальное состояние Sprint 15: primary/only source corpus remains Вронский, Том 1, Приложение 2, Таблица 17 — Арабские точки; default active formulas remain `pars-fortuna` and `lot-of-spirit`; selected Vronsky rows remain the 12 explicit day-only display-safe rows; night formulas remain `missing/notVerified`;
- deferred remain inactive: `lot-of-eros`, `lot-of-necessity`, `lot-of-basis`, `lot-of-exaltation`, sensitive/deferred Vronsky rows, complex operand rows and night Vronsky formulas;
- Task 16.1 завершена: added / accepted `SPRINT_16_PLAN.md`, `MIDPOINTS_ANTISCIA_STRATEGY.md` and `MIDPOINTS_ANTISCIA_SOURCE_POLICY.md` as Sprint 16 strategy docs;
- Task 16.2 завершена: added `MIDPOINTS_ANTISCIA_FORMULA_SCOPE_DECISION.md` and accepted source status `source-verified-for-implementation`;
- midpoint policy: shortest-arc midpoint between normalized tropical longitudes; exact 180° opposition is `exactOpposition` / `midpointAxisAmbiguous` and must not be silently reduced to one arbitrary point;
- antiscion policy: `normalize(180 - longitude)`, mirror across the Cancer / Capricorn solstice axis;
- contra-antiscion policy: `normalize(360 - longitude)`, mirror across the Aries / Libra equinox axis;
- active target scopes: Midpoints use natal planets only, expected pair count `45`; Antiscia / Contra-antiscia use natal planets + ASC / MC / DSC / IC, expected target count `14`;
...
- Task 16.3 завершена: added pure `src/midpointTargets.js` target resolver, static/manual fixtures and tests; active midpoint targets are natal planets only, target count is `10`, deterministic unordered pair definitions count is `45`, and deferred target sets remain metadata-only;
- Task 16.4 завершена: added pure `src/midpoints.js` midpoint engine, static/manual fixtures and tests; active midpoint scope remains natal planets only, pair count is `45`, shortest-arc midpoint calculation is implemented, wrap-around through `0° Aries` is supported, and exact 180° opposition returns `axisAmbiguous` / `midpointAxisAmbiguous` without an arbitrary primary longitude;
- Task 16.5 завершена: added static/manual midpoint validation fixtures and cross-check tests for target scope, pair definitions, shortest-arc formula, wrap-around, exact opposition ambiguity, batch counts, deferred scopes, privacy and strict exclusions;
- Task 16.6 завершена: added pure `src/antisciaTargets.js` target resolver, static/manual fixtures and tests; active antiscia targets are natal planets + ASC / MC / DSC / IC, target count is `14`, and deferred target sets remain metadata-only;
- no formula from memory, hidden formula policy, interpretations, transits, ritual scoring, midpoint contacts, midpoint pictures, antiscia engine, contra-antiscia engine, UI or debug were added in Task 16.1 / Task 16.2 / Task 16.3 / Task 16.4 / Task 16.5 / Task 16.6;
- Task 16.7 завершена: добавлен pure `src/antiscia.js` engine для antiscion `normalize(180 - longitude)` и contra-antiscion `normalize(360 - longitude)`, scope остался natal planets + ASC / MC / DSC / IC, target count `14`; следующая активная задача — Task 16.8 — Antiscia Validation / Cross-Checks; Task 16.8 не начата.
...
- Task 13.6 завершена: добавлен pure `src/lilith.js` Mean Lilith engine, static `SE_MEAN_APOG` benchmark fixtures and tests; implemented only Mean Lunar Apogee; no true/osculating/interpolated Lilith, Selena, house assignment, UI/debug or interpretations were added;
- Task 13.7 завершена: добавлен `SELENA_SOURCE_DECISION.md`; Selena status is `source-verified-for-implementation`; active source system is `selena-white-moon`; calculation method is `swisseph-seorbel-white-moon-linear-elements`; Selena is treated as a Swiss Ephemeris fictitious / hypothetical calculated point, not a physical astronomical body; local Swiss Ephemeris `SE_WHITE_MOON` / `swe_calc_ut` is the static benchmark oracle only; no Selena engine, fixtures with calculated values, UI/debug or interpretations were added;
- Task 13.8 завершена: добавлен pure `src/selena.js` Selena / White Moon engine, static `SE_WHITE_MOON` benchmark fixtures and tests; implemented only the selected Swiss Ephemeris seorbel White Moon source system; Selena is marked as `fictitious-calculated-point`; no alternate Selena variants, Lunar Nodes changes, Lilith changes, house assignment, UI/debug or interpretations were added;
- Task 13.9 завершена: добавлен pure `src/specialPointsDisplay.js` display helper for already calculated Special Points; it formats North/South Nodes, Mean Lilith, Selena, optional Lunar Nodes house labels, safe fallback states and the Selena fictitious / hypothetical note without calculating points, assigning houses, rendering UI/debug or adding interpretations;
- Task 13.10 завершена: добавлен collapsible UI block `Особые точки карты` inside `Мои карты` after `Жребии и арабские части`; block is collapsed by default, shows Lunar Nodes, Mean Lilith and Selena when ready, shows node house assignment when ready, includes the safe Selena fictitious / hypothetical calculated-point note, and no interpretations/debug were added;
- Task 13.11 завершена: добавлен safe `Special Points UI Debug` for `?debug=1`; debug shows only active profile id/name, readiness booleans, source statuses, counts, capabilities, collapsed state and privacy flags, without raw birth data, coordinates, UTC, raw longitudes, full profile JSON, full points/assignments/cusps arrays or provider payloads;
- Task 13.12 завершена: Sprint 13 hardening closed the Special Points Foundation after rechecking mean Lunar Nodes, Lunar Nodes house assignment, Mean Lilith, Selena / White Moon, UI/debug, privacy and architecture boundaries; true node, true / osculating / interpolated Lilith and alternate Selena source systems remain deferred; Fixed Stars, Arabic Parts Expansion, Midpoints, transits, interpretations and ritual scoring were not added;
- Task 14.1 завершена: added / accepted Sprint 14 Fixed Stars strategy docs and recorded source-gated catalog, coordinate/epoch/precession, explicit orb and conjunction-only initial policies;
- Task 14.2 завершена: added Fixed Stars source/catalog/orb policy; primary source is Vronsky Table 18 with Swiss / modern validation where possible; candidate-only subset is Алголь, Альдебаран, Ригель, Бетельгейзе, Сириус, Канопус, Регул, Спика, Арктур, Антарес, Вега, Альтаир and Фомальгаут; global conjunction orb is `1°00′`; first target set is natal planets + ASC / MC / DSC / IC;
- Task 14.3 завершена: added source-tracked `src/fixedStarsData.js` catalog dataset, fixture file and tests; 13 active rows were manually verified from Vronsky Table 18 reference photos; candidate row count is 0; no Fixed Stars position engine, conjunction engine, UI/debug or interpretations were added;
- Task 14.4 завершена: added pure `src/fixedStarPositions.js` position / epoch engine and fixtures; source epochs 1950 / 1970 / 1990 are exact, dates between source epochs are interpolated, dates outside the source range are extrapolated explicitly, and no conjunction engine, target resolver, UI/debug or interpretations were added;
- Task 14.5 завершена: added pure `src/fixedStarTargets.js` target resolver for natal planets and ASC / MC / DSC / IC; deferred target sets remain metadata-only, and no conjunction engine, UI/debug or interpretations were added;
- Task 14.6 завершена: added pure `src/fixedStarConjunctions.js` conjunction engine for natal planets + ASC / MC / DSC / IC targets only; relationship is conjunction-only, global orb is `1°00′`, wrap-around and inclusive boundary are supported, and no UI/debug or interpretations were added;
- Task 14.7 завершена: added Fixed Stars validation / cross-check fixtures and tests for catalog, positions, targets, conjunctions, privacy and strict exclusions; no UI/display/debug or interpretations were added;
- Task 14.8 завершена: added pure `src/fixedStarsDisplay.js` display helper for already calculated conjunction hits, noHits/fallback states and safe source/orb notes; no UI/debug or interpretations were added;
- Task 14.9 завершена: added collapsible `Неподвижные звезды` UI after `Особые точки карты`, backed by `src/fixedStarConjunctions.js` and `src/fixedStarsDisplay.js`; no debug or interpretations were added;
- Task 14.10 завершена: added safe `Fixed Stars Debug` for `?debug=1`; no raw profile data, full arrays, provider payloads or interpretations are exposed;
- Task 14.11 завершена: Sprint 14 final hardening / regression passed without code fixes;
- Sprint 14 закрыт;
- Task 15.1 завершена: Sprint 15 Arabic Parts Expansion Pack strategy docs accepted; Task 15.2 завершена as blocked/source-materials-required; Task 15.2b завершена as Vronsky Table 17 source-materials intake; Task 15.2c завершена as Vronsky Arabic Points scope selection; Task 15.3 завершена as Vronsky simple Arabic Points dataset / fixtures; Task 15.4 завершена as Vronsky simple Arabic Points explicit engine / fixtures; Task 15.5 завершена as Vronsky Arabic Points house assignment / display integration; Task 15.6 завершена as Vronsky Arabic Points UI update inside the existing `Жребии и арабские части` block; Task 15.7 завершена as Vronsky Arabic Points Debug Update; Task 15.8 завершена as Vronsky Arabic Points Final Hardening / Regression; Sprint 15 закрыт; Task 16.1 завершена as Midpoints / Antiscia Strategy; Task 16.2 завершена as Midpoints / Antiscia Source / Formula / Scope Policy; Task 16.3 завершена as Midpoint Target Resolver / Fixtures; Task 16.4 завершена as Midpoint Engine / Fixtures; Task 16.5 завершена as Midpoint Validation / Cross-Checks; Task 16.6 завершена as Antiscia Target Resolver / Fixtures; Task 16.7 завершена as Antiscia / Contra-antiscia Engine / Fixtures; следующая активная задача — Task 16.8 — Antiscia Validation / Cross-Checks;
...
Sprint 8 завершен. Task 8.1, Task 8.2, Task 8.3, Task 8.4, Task 8.5, Task 8.6 и Task 8.7 завершены. Sprint 9 завершен: Task 9.1, Task 9.2, Task 9.3, Task 9.4, Task 9.5, Task 9.6, Task 9.7 и Task 9.8 закрыты. Sprint 10 закрыт: Task 10.1, Task 10.2, Task 10.3a, Task 10.3b, Task 10.3c, Task 10.4, Task 10.5, Task 10.5b, Task 10.5c, Task 10.5d, Task 10.6, Task 10.7a, Task 10.7b, Task 10.7c, Task 10.7d, Task 10.8, Task 10.9, Task 10.8b, Task 10.8c, Task 10.8d, Task 10.8d-fix, Task 10.8e, Task 10.8f, Task 10.9b, Task 10.10, Task 10.11 и Task 10.12 закрыты. Sprint 11 закрыт: Task 11.1 закрыла Houses / ASC / MC strategy docs, Task 11.2 добавила Birth Input / Coordinates Guardrails, Task 11.3 добавила ASC / MC Calculation Engine, Task 11.4a закрыла House Systems Strategy / Dependency Audit, Task 11.4b добавила Whole Sign Houses Engine, Task 11.4c добавила Equal House Engine, Task 11.4d добавила Placidus integration gate, Task 11.4d2 активировала Placidus calculation через статические benchmark fixtures, Task 11.4e добавила House System Resolver / Selected System Router, Task 11.5 добавила Houses Validation / Fixtures for Whole Sign / Equal House / Placidus, Task 11.6 добавила Planet-in-House Assignment, Task 11.7 добавила Houses / ASC / MC Display Helper, Task 11.8 добавила collapsible `Дома и углы карты` UI block, Task 11.8b добавила ручной ввод координат места рождения, Task 11.8c проверила Placidus ready state and added duplicate-cusp regression tests, Task 11.9 добавила safe Houses / ASC / MC debug, а Task 11.10 завершила final Sprint 11 hardening. Sprint 12 закрыт: Task 12.1 добавила strategy docs and formula policy; Task 12.2 добавила canonical house cusp output layer and fixtures; Task 12.3 добавила day/night chart status engine and fixtures; Task 12.4 добавила Pars Fortuna engine and fixtures; Task 12.5 добавила Arabic Parts formula dataset/source decision; Task 12.5b verified Lot of Spirit as the second active formula row; Task 12.6 added Basic Arabic Parts engine and fixtures for active verified formulas only; Task 12.7 added Lots / Arabic Parts house assignment for active verified lots; Task 12.8 added Lots / Arabic Parts display helper for already calculated results; Task 12.9 added the user-facing Arabic Parts UI block; Task 12.10 added safe Arabic Parts debug; Task 12.11 completed final Sprint 12 hardening with no code fixes needed. Sprint 13 closed after Special Points UI/debug and hardening. Sprint 14 closed: Task 14.1 and Task 14.2 completed strategy/source policy; Task 14.3 added source-tracked Fixed Stars catalog data and fixtures; Task 14.4 added pure Fixed Star position / epoch engine with exact-source, interpolation, extrapolation and wrap-around handling; Task 14.5 added pure Fixed Star target resolver for natal planets and angles; Task 14.6 added pure Fixed Star conjunction engine for conjunction-only detection with global `1°00′` orb; Task 14.7 added Fixed Stars validation / cross-check fixtures and tests; Task 14.8 added pure Fixed Stars display helper; Task 14.9 added the user-facing `Неподвижные звезды` UI block after `Особые точки карты`; Task 14.10 added safe Fixed Stars Debug / QA guardrails; Task 14.11 completed final hardening / regression without code fixes. Sprint 15 started with Task 15.1 docs-only strategy: `SPRINT_15_PLAN.md`, `ARABIC_PARTS_EXPANSION_STRATEGY.md` and `ARABIC_PARTS_EXPANSION_SOURCE_POLICY.md`; active formulas remain Pars Fortuna and Lot of Spirit; Task 15.2 added `ARABIC_PARTS_SOURCE_CORPUS_DECISION.md`; Task 15.2b accepted Vronsky Table 17 as the partial primary/only source corpus and recorded source-visible day-birth rows without activating new Lots; Task 15.2c added `ARABIC_PARTS_VRONSKY_SCOPE_SELECTION.md`, selected the first Vronsky simple day-only display-safe implementation pack, deferred sensitive labels and complex operands, kept night formulas missing/unverified and activated no new Lots; Task 15.3 added 12 inactive pending-engine Vronsky simple Arabic Points dataset rows and fixtures without engine expansion. Task 15.4 added the explicit day-only Vronsky simple Arabic Points engine / fixtures; Task 15.5 added explicit Vronsky house assignment / display integration and the explicit profile helper without UI/debug; Task 15.6 added the Vronsky Arabic Points UI subsection inside the existing `Жребии и арабские части` block; Task 15.7 added safe Vronsky Arabic Points Debug / QA guardrails for `?debug=1`; Task 15.8 completed final Vronsky Arabic Points hardening / regression; Sprint 15 is closed; Task 16.1 started Sprint 16 as a docs-only strategy task; Task 16.2 completed the Midpoints / Antiscia source, formula and scope policy; Task 16.3 added the pure midpoint target resolver / fixtures for 10 natal planet targets and 45 deterministic pair definitions; Task 16.4 added the pure midpoint engine and fixtures; Task 16.5 added midpoint validation/cross-check fixtures and tests; Task 16.6 added pure `src/antisciaTargets.js` target resolver / fixtures for natal planets + ASC / MC / DSC / IC, target count `14`, without antiscia calculations; Task 16.7 added pure `src/antiscia.js` Antiscia / Contra-antiscia engine and fixtures for the same active target scope; next active task is Task 16.8 — Antiscia Validation / Cross-Checks.
...
2. Не начинать Task 15.7 без отдельной команды пользователя.
3. Объяснить минимальный план изменения.
4. Внести только необходимые изменения.
5. Не активировать новые Arabic Parts без source/tradition verification.
