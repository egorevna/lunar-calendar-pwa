...
Доработать блок `Луна без курса`, чтобы он показывал не просто сырой интервал, а понятное состояние момента.

Нужно:

Если VOC еще не началась:
...
или, если лучше помещается в интерфейс:
...
- на главном экране время показывать до минут;
- секунды убрать;
- секунды допустимы только в техническом/debug режиме.
...
- если VOC еще не началась, показывается правильный заголовок, интервал и обратный отсчет;
- если VOC идет, показывается правильный заголовок, время окончания и обратный отсчет;
- если VOC сегодня нет, показывается `Луна в курсе` и следующая VOC;
- строка `после: ...` заменена на понятную формулировку;
- на главном экране нет секунд в VOC-времени;
- существующие расчеты Луны не сломаны;
- тесты проходят.
...
После Task 1.1 не переходить автоматически дальше.

Следующие задачи должны выполняться только по отдельной команде пользователя.

## Task 1.2 — Add VOC Quality Label

Добавить краткую оценку качества VOC:

- после гармоничного аспекта: `мягкий VOC`;
- после напряженного аспекта: `напряженный VOC`;
- после аспекта с Нептуном: `размытый VOC`;
- после аспекта с Сатурном: `тяжелый VOC`;
- после аспекта с Марсом / Ураном: `нервный VOC`.
...
```txt
Выполнить Task 1.1 — Fix VOC Block States and Copy.
```

Перед реализацией Codex должен:

1. Найти, где сейчас отображается блок `Луна без курса`.
2. Найти, откуда берется VOC-данные.
