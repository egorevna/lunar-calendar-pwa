...
- блок `Луна без курса` теперь показывает разные состояния для upcoming / active / none;
- время в VOC-блоке на главном экране показывается до минут, без секунд;
- добавлен обратный отсчет до начала или окончания VOC;
- строка последнего аспекта заменена на `VOC после: ...`;
...
# Upcoming Sprint 1 Tasks After Active Task

После Task 1.2 не переходить автоматически дальше.
...
```txt
Выполнить Task 1.2 — Add VOC Quality Label.
```
